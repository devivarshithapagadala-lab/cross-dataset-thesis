...
def in_domain_baseline_for_supervised_models_2018():
    print("Training and testing of supervised models on same dataset 'CSE-CIC-IDS2018'")
    ...
    print(f"\nshape of the train raw data- {train_df.shape}")
    print(f"shape of the test raw data- {test_df.shape}")
    ...
    output_dir = "results/studies/analysis/in_domain_baseline_supervised_2018"
    os.makedirs(output_dir, exist_ok=True)
    for name, model in models.items():
        print(f"\nTraining the model {name} on the same dataset CSE-CIC-IDS2018")
        model.fit(x_train, y_train.values)
        print(f"Evaluation of the model {name} on the sane dataset CSE-CIC-IDS2018")
        model.evaluate(x_test, y_test.values, output_dir=output_dir)
    print("\nBaseline for the in domain of  unsupervised baseline has finished")

if __name__ == "__main__":
    in_domain_baseline_for_supervised_models_2018()