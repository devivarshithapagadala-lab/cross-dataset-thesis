...
Fractions_of_few_shot = [0.0, 0.01, 0.02, 0.05, 0.10]
Type_of_models = ["Random_Forest", "Gradient_Boosting", "Logistic_Regression", "Neural_Network"]

def experiment_of_few_shot(direction_name, x_train_source, y_train_source, x_target_full, y_target_full, output_dir):
    print(f"\nFine tuning on few shot - {direction_name}")
    os.makedirs(output_dir, exist_ok=True)
    few_shot_pool_of_x, evaluation_of_x, few_shot_pool_of_y, evaluation_of_y = train_test_split(x_target_full, y_target_full, test_size=0.5, random_state=42, stratify=y_target_full)
    print(f"pool of few shot {few_shot_pool_of_x.shape[0]} samples & evaluation on held out {evaluation_of_x.shape[0]} samples")
    results = []
    for model_type in Type_of_models:
        ...
        for fraction in Fractions_of_few_shot:
            ...
            print(f"fraction target - {fraction:.2f} ({n_target_samples_used} samples)"
    ...
    results_df.to_csv(os.path.join(output_dir, "few_shot_sweep.csv"), index=False)
    best_per_model = results_df.loc[results_df.groupby("Model")["F1"].idxmax()]
    print(f"\nconfiguration per model which is best for the direction{direction_name}:")
    print(best_per_model[["Model", "Target_Fraction", "Target_Samples_Used", "Precision", "Recall", "F1"]])
    best_per_model.to_csv(os.path.join(output_dir, "few_shot_best_summary.csv"), index=False)
    return results_df

def main():
    base_out = "results/studies/improvements/supervised/improvement_few_shot_finetuning_all_directions"
    all_results = []
    ...
    r = experiment_of_few_shot("CICIDS2017 towards CSE-CIC-IDS2018", x_train_of_2017, y_train_of_2017, x_test_of_2018, y_test_of_2018, os.path.join(base_out, "2017_to_2018"))
    ...
    r = experiment_of_few_shot("UNSW-NB15 towards CICIDS2017", x_train_of_unsw, y_train_of_unsw, x_test_of_2017_b, y_test_of_2017_b, os.path.join(base_out, "unsw_to_2017"))
    ...
    r = experiment_of_few_shot("UNSW-NB15 towards CSE-CIC-IDS2018", x_train_of_unsw, y_train_of_unsw, x_test_of_2018_b, y_test_of_2018_b, os.path.join(base_out, "unsw_to_2018"))
    r["Direction"] = "unsw_to_2018"
    all_results.append(r)

    combined = pd.concat(all_results, ignore_index=True)
    combined.to_csv(os.path.join(base_out, "few_shot_all_directions_summary.csv"), index=False)
    print(f"\nStudy of few shot fine tuning has finished\n")
    best_overall = combined.loc[combined.groupby(["Direction", "Model"])["F1"].idxmax()]
    print(best_overall[["Direction", "Model", "Target_Fraction", "Target_Samples_Used", "F1"]])
